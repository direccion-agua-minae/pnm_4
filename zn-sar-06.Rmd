...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,3])),
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,2])),
...
### Cuenca {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,1])), 
         caption = "Cuenca",
         color = color_info
)
```
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,12])), 
...
### Coordenadas {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,13])), 
...
### Provincia {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,15])), 
         caption = "Provincia",
         color = color_ubic
)
```
...
### Cantón {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,16])), 
         caption = "Cantón",
         color = color_ubic
)
```
...
### Distrito {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[117,17])), 
         caption = "Distrito",
         color = color_ubic
)
```
...
```{r}
knitr::kable(
  archivo_datos_PNM[117:120, 18:24], bootstrap_options = "basic", 
...
leaflet(data = PNM[152:152,]) %>%
  addTiles() %>%
  addProviderTiles(providers$Stamen.TonerLite, group = "Stamen Toner Lite") %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Imágenes de ESRI") %>% 
  addMarkers(~LONG, ~LAT, 
             popup= paste(
               "Sitio de muestreo: ", PNM$Codigo[152]
  )) %>%  
  addLayersControl(
