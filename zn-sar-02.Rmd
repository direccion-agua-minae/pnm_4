...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,3])),
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,2])),
...
### Cuenca {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,1])), 
         caption = "Cuenca",
         color = color_info
)
```
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,12])), 
...
### Coordenadas {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,13])), 
...
### Provincia {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,15])), 
         caption = "Provincia",
         color = color_ubic
)
```
...
### Cantón {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,16])), 
         caption = "Cantón",
         color = color_ubic
)
```
...
### Distrito {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[101,17])), 
         caption = "Distrito",
         color = color_ubic
)
```
...
```{r}
knitr::kable(
  archivo_datos_PNM[101:104, 18:24], bootstrap_options = "basic", 
...
leaflet(data = PNM[148:148,]) %>%
  addTiles() %>%
  addProviderTiles(providers$Stamen.TonerLite, group = "Stamen Toner Lite") %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Imágenes de ESRI") %>% 
  addMarkers(~LONG, ~LAT, 
             popup= paste(
               "Sitio de muestreo: ", PNM$Codigo[148]
  )) %>%  
  addLayersControl(
